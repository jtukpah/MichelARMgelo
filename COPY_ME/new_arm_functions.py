...
# These are necessary functions for our code to interact with the interbotix backend.
# Ensure the indentation is correct for these functions to be included in the InterbotixArmXSInterface class.

    # Draw a smooth line connecting two points on the outer surface of a sphere.
    ...
    # Draw a smooth line connecting two points on the inner surface of a sphere.
    # When used with points on a sphere, only monotonic changes are possible.
    # i.e., X cannot increase and then decrease in the same motion.
    def set_ee_inner_arc_trajectory(self, dx=0.2, dz=0.3, dr=0, dp=0, PEN_OFFSET = 0.055, moving_time=None, wp_moving_time=0.2, wp_accel_time=0.1, wp_period=0.05):
        rpy = ang.rotationMatrixToEulerAngles(self.T_sb[:3,:3])
        T_sy = np.identity(4)
        T_sy[:3,:3] = ang.eulerAnglesToRotationMatrix([0.0, 0.0, rpy[2]])
        T_yb = np.dot(mr.TransInv(T_sy), self.T_sb)
        ...
        for i in range(N+1):
            ...
            if (i == N):
                break
            # T_yb[:3,3] += [math.sin(i/N*math.pi/2) * x, inc * y, math.sin(i/N*math.pi/2) * z]
            T_yb[:3,3] = [x0 + dx * math.sin(i/N*math.pi/2), 0, (z0+dz) - dz*math.cos(i/N*math.pi/2)]
            rpy[0] += inc * dr
            rpy[1] += inc * dp
            T_yb[:3,:3] = ang.eulerAnglesToRotationMatrix(rpy)
            T_sd = np.dot(T_sy, T_yb)
